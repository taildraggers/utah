...
Daily aggregator of Utah-located taildragger classified listings,
...
- Each fetched page's listing table is parsed back into `(title, url,
  ...
  Location cell ends in `, UT` (case-insensitive), then de-duplicated by
...
- The result is rendered into `docs/index.html` titled **"Taildragger Ads
  in Utah"**, using the exact same table/card layout, dark-mode
...
4. GitHub will publish the page at `https://taildraggers.github.io/utah/`
...
```html
<iframe
  src="https://taildraggers.github.io/utah/"
  title="Taildragger Ads in Utah"
...
- This repo could be copied/adapted for other states by changing
  `STATE_ABBR` and `_UTAH_LOCATION_RE` in `main.py`, and updating
  `PAGE_TITLE`.
