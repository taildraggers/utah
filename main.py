"""Aggregates Utah-located listings from every taildraggers.com
...
ones whose location ends in ", UT".
...
repos' approach, with the state filter swapped to "UT".
"""
...
PAGE_TITLE = "Taildragger Ads in Utah"
STATE_ABBR = "UT"
...
_UTAH_LOCATION_RE = re.compile(r",\s*UT\s*$", re.IGNORECASE)


def _is_utah(location: str) -> bool:
    return bool(_UTAH_LOCATION_RE.search(location.strip()))
...
def collect_listings() -> list[Listing]:
    ...
    for repo in SOURCE_SITES:
        ...
        utah_rows = [row for row in rows if _is_utah(row.location)]
        print(f"  [{repo}] {len(rows)} listings, {len(utah_rows)} in Utah")
        all_listings.extend(utah_rows)
...
def render_html(listings: list[Listing]) -> str:
    ...
    rows_html = "".join(rows) if rows else (
        ...
        "No Utah listings found in the latest run.</td></tr>"
    )
...
def main() -> None:
    ...
    print(f"[main] total unique Utah listings: {len(listings)}")
